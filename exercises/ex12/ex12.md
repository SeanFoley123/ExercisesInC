...
- At 5000 threads, I get some synchronization errors - I lost 1 thread in there somewhere.
- The issue was probably that another process was executed between retrieving the variable (shared->counter) and incrementing it (++)
...
- Roughly an order of magnitude. Interestingly, for counter_array, there's pretty much no "sys" time, while for counter_array_mutex, it's about a third of the time spent.